...
vi.mock('node:module', () => ({
  createRequire: (_url: string | URL) => ({
    resolve: (specifier: string, _options?: { paths?: string[] }) => {
      if (specifier === 'legacy-lib/package.json') {
        return '/mocks/legacy-lib/package.json';
      }

      if (specifier === 'other-lib/package.json') {
        return '/mocks/other-lib/package.json';
      }
...
const file = (name: string): DirentLike => ({ name, isFile: () => true, isDirectory: () => false });
const dir = (name: string): DirentLike => ({ name, isFile: () => false, isDirectory: () => true });
...
const setupReaddirMock = () => {
  const impl = (path: string): DirentLike[] => {
    if (path === '/mocks/legacy-lib/lib') {
      return [file('Button.js'), file('Input.js'), dir('Grid')];
    }

    if (path === '/mocks/legacy-lib/lib/Grid') {
      return [file('Column.js'), file('Row.js')];
    }

    if (path === '/mocks/other-lib/dist') {
      return [file('Widget.js')];
    }

    return [];
  };

  vi.mocked(readdirSync).mockImplementation(impl as unknown as typeof readdirSync);
};
...
describe('legacyInterop', () => {
  ...
  describe('factory', () => {
    ...
    it('ignores empty strings and uses valid entries', () => {
      ...
      ).toBe('\0legacy-interop:legacy-lib/lib/Button');
    });
  });
  ...
  describe('resolveId', () => {
    ...
    it('returns null for import without libDir prefix', () => {
      ...
      expect(
        resolveId.call({} as never, 'legacy-lib/Button', undefined, { isEntry: false }),
      ).toBeNull();
    });
    ...
    it('resolves flat component with .js extension', () => {
      ...
      expect(
        resolveId.call({} as never, 'legacy-lib/lib/Button.js', undefined, { isEntry: false }),
      ).toBe('\0legacy-interop:legacy-lib/lib/Button.js');
    });
    ...
    it('resolves nested component (Grid/Column)', () => {
      ...
      expect(
        resolveId.call({} as never, 'legacy-lib/lib/Grid/Column', undefined, { isEntry: false }),
      ).toBe('\0legacy-interop:legacy-lib/lib/Grid/Column');
    });
    ...
    it('returns null when importer is our own virtual module (loop prevention)', () => {
      ...
      expect(
        resolveId.call({} as never, 'legacy-lib/lib/Button.js', virtualImporter, {
          isEntry: false,
        }),
      ).toBeNull();
    });
    ...
    it('returns null and warns for unknown module', () => {
      ...
      const plugin = legacyInterop({ libs: ['legacy-lib'] });
      const resolveId = getResolveId(plugin);
      const result = resolveId.call({} as never, 'legacy-lib/lib/NonExistent', undefined, {
        isEntry: false,
      });
    ...
    it('handles multiple libs', () => {
      ...
      expect(
        resolveId.call({} as never, 'other-lib/dist/Widget', undefined, { isEntry: false }),
      ).toBe('\0legacy-interop:other-lib/dist/Widget');
    });
    ...
    it('resolves import with a query suffix (e.g. ?raw)', () => {
      ...
      expect(
        resolveId.call({} as never, 'legacy-lib/lib/Button?raw', undefined, { isEntry: false }),
      ).toBe('\0legacy-interop:legacy-lib/lib/Button?raw');
    });
    ...
    it('uses custom libDir', () => {
      ...
      expect(
        resolveId.call({} as never, 'other-lib/lib/Widget', undefined, { isEntry: false }),
      ).toBeNull();
    });
  });
  ...
  describe('error handling', () => {
    ...
    it('warns distinctly when the package resolves but libDir does not exist', () => {
      const impl = (path: string): DirentLike[] => {
        if (path === '/mocks/legacy-lib/missing-dir') {
